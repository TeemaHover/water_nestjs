...
export class EventService {
  ...
  async getEvent() {
    try {
      return await this.model
        .find({ endDate: { $gte: Date.now() } })
        .populate('business', 'id  companyName', this.business);
    } catch (error) {
      throw new HttpException(error.message, 500);
    }
  }
  ...
  async addMemberEvent(id: string, user: any) {
    try {
      ...
      let isEvent = await this.model.findOne({ users: { $in: [user] } });
      if (isEvent) throw new HttpException('Registered user', 400);
      if (event.members <= event.registerMembers)
        throw new HttpException('Event is full', 400);
      event = await this.model.findByIdAndUpdate(id, {
  ...
  async getEventByBusiness(id: string, user: UserType) {
    try {
      if (user === UserType.user)
        return await this.model.find({ business: id }).select('-users');
